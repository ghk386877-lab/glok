...
HELP_TEXT = (
    ...
    "/snipe <назва> - шукає недооцінені по рідкості лоти в колекції (напр. /snipe Toy Bear)\n"
    "/snipe_top - те саме одразу для топових ліквідних подарунків\n"
    "/mrkt_orders <назва> - те саме, що /orders, але для ордерів на MRKT\n"
    "/mrkt_orders_top - топ MRKT-ордерів за прибутком\n"
    "/buy <назва> <ціна> - записати купівлю (напр. /buy Toy Bear 32.5)\n"
    "/sell <назва> <ціна> - записати продаж, закриває найстарішу відкриту позицію\n"
    "/portfolio - твій портфель: реалізований і нереалізований прибуток\n"
...
async def cmd_orders_top(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        text = await orders.build_profit_ranked_report(candidate_names, top_n=config.ORDERS_TOP_N)
    except Exception as e:
        text = f"Помилка: {type(e).__name__}: {e}"
    await update.message.reply_text(text)


async def cmd_mrkt_orders(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    import mrkt_orders
    import mrkt_client
    ...
    if not mrkt_client.is_connected():
        ...
        return
    if not context.args:
        await update.message.reply_text("Вкажи назву подарунка, наприклад: /mrkt_orders Crystal Ball")
        return
    gift_name = " ".join(context.args)
    await update.message.reply_text(f"Дивлюсь MRKT-ордери для '{gift_name}'...")
    try:
        text = await mrkt_orders.build_mrkt_order_report(gift_name)
    except Exception as e:
        text = f"Помилка: {type(e).__name__}: {e}"
    await update.message.reply_text(text)


async def cmd_mrkt_orders_top(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    import mrkt_orders
    import mrkt_client
    ...
    for name in names:
        latest = await storage.get_latest(name)
        if latest and (latest["sales_24h_count"] or 0) >= config.MIN_SALES_24H_FOR_VALID:
            rows.append((name, latest["volume_24h"] or 0))
    rows.sort(key=lambda r: r[1], reverse=True)
    candidate_names = [name for name, _ in rows[: config.ARBITRAGE_CANDIDATE_POOL]]

    if not candidate_names:
        await update.message.reply_text("Немає достатньо ліквідних подарунків для сканування.")
        return

    await update.message.reply_text(
        f"Перевіряю MRKT-ордери для {len(candidate_names)} ліквідних подарунків, зачекай..."
    )
    try:
        text = await mrkt_orders.build_mrkt_order_top_report(candidate_names)
    except Exception as e:
        text = f"Помилка: {type(e).__name__}: {e}"
    await update.message.reply_text(text)
...
async def cmd_snipe(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    import rarity_snipe

    if not context.args:
        await update.message.reply_text("Вкажи назву подарунка, наприклад: /snipe Toy Bear")
        return
    gift_name = " ".join(context.args)
    await update.message.reply_text(f"Скануюю лістинги '{gift_name}' на рідкісні недооцінені лоти...")
    try:
        text = await rarity_snipe.build_snipe_report(gift_name)
    except Exception as e:
        text = f"Помилка: {type(e).__name__}: {e}"
    await update.message.reply_text(text)


async def cmd_snipe_top(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    import rarity_snipe
    ...
    for name in names:
        ...
        if latest and (latest["sales_24h_count"] or 0) >= config.MIN_SALES_24H_FOR_VALID:
            rows.append((name, latest["volume_24h"] or 0))
    rows.sort(key=lambda r: r[1], reverse=True)
    candidate_names = [name for name, _ in rows[: config.SNIPE_CANDIDATE_POOL]]
    ...
    await update.message.reply_text(
        f"Скануюю {len(candidate_names)} ліквідних колекцій на снайпи по рідкості, зачекай..."
    )
    try:
        text = await rarity_snipe.build_snipe_top_report(candidate_names)
    except Exception as e:
        text = f"Помилка: {type(e).__name__}: {e}"
    await update.message.reply_text(text)
...
async def cmd_buy(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if len(context.args) < 2:
        await update.message.reply_text(
            "Використання: /buy <назва> <ціна>, напр. /buy Toy Bear 32.5"
        )
        return
    try:
        price = float(context.args[-1])
    except ValueError:
        await update.message.reply_text("Остання частина команди має бути числом (ціною).")
        return
    gift_name = " ".join(context.args[:-1])
    chat_id = update.effective_chat.id
    trade_id = await storage.add_purchase(chat_id, gift_name, price)
    await update.message.reply_text(f"Записав купівлю #{trade_id}: {gift_name} за {price:.2f} TON.")


async def cmd_sell(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if len(context.args) < 2:
        await update.message.reply_text(
            "Використання: /sell <назва> <ціна>, напр. /sell Toy Bear 35.0"
        )
        return
    try:
        price = float(context.args[-1])
    ...
    if not result:
        await update.message.reply_text(
            f"Не знайшов відкритої позиції '{gift_name}' для продажу. "
            f"Перевір назву або спочатку /buy."
        )
        return
    sign = "\U0001f7e2" if result["profit"] > 0 else "\U0001f534"
    await update.message.reply_text(
        f"{sign} Продав #{result['id']}: {gift_name} за {price:.2f} TON "
        f"(купив за {result['buy_price']:.2f}), прибуток {result['profit']:+.2f} TON."
    )
...
async def cmd_portfolio(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    chat_id = update.effective_chat.id
    text = await portfolio.build_portfolio_report(chat_id)
    await update.message.reply_text(text)


async def cmd_delete_trade(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not context.args:
        await update.message.reply_text("Використання: /delete_trade <id> (id з /portfolio)")
        return
    try:
    ...
    await update.message.reply_text(
        f"Видалено угоду #{trade_id}." if ok else "Не знайшов таку угоду у твоєму портфелі."
    )
