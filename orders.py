...
async def _fetch_offer_profit(sem: asyncio.Semaphore, name: str):
    ...
    Повертає ("ok", ...), ("auth_error", name) або None (немає офера)."""
    async with sem:
        ...
        net_floor_sale = floor * (1 - config.PORTALS_FEE_PERCENT / 100)
        profit = net_floor_sale - amount
        return ("ok", name, amount, floor, profit)
...
async def build_profit_ranked_report(gift_names: list[str], top_n: int | None = None) -> str:
    ...
    Якщо authData протух саме для цих ендпоінтів (окрема, суворіша перевірка
    свіжості підпису на стороні Portals) - бот сам один раз примусово оновлює
    токен через portals_auth і повторює спробу, перш ніж здатись.
    """
    top_n = top_n or config.TOP_N_DISPLAY

    async def _run_batch():
        sem = asyncio.Semaphore(config.API_CONCURRENCY_LIMIT)
        return await asyncio.gather(*(_fetch_offer_profit(sem, name) for name in gift_names))

    raw = await _run_batch()
    ok_results = [r[1:] for r in raw if r and r[0] == "ok"]
    auth_errors = sum(1 for r in raw if r and r[0] == "auth_error")

    if not ok_results and auth_errors > 0:
        # authData протух саме для цих ендпоінтів - пробуємо самі відновити і повторити,
        # замість того щоб показувати оманливе "немає валідних подарунків"
        import portals_auth
    ...
    if not ok_results:
        if auth_errors > 0:
            return (
                "\u26a0\ufe0f authData протух саме для ендпоінтів оферів, і "
                "автоматичне відновлення не допомогло. Спробуй /refreshauth "
                "вручну, або перевір, чи налаштовано MRKT_API_ID/HASH і чи "
                "виконаний setup_portals_auth.py."
            )
        return (
