...
class PortalsAuthError(PortalsError):
    """authData протух/невалідний саме для цього запиту (401 auth sign is invalid)."""
    pass
...
async def search_listings(
    ...
    """
    Обхід aportalsmp.search(): та функція перевіряє gift_name через
    застарілий словник collections_ids (та сама проблема, що й у topOffer -
    підтверджено 16.07.2026, лише ~105 колекцій). Тут будуємо URL так само,
    як бібліотека всередині (модуль aportalsmp.gifts), але підставляємо
    РЕАЛЬНИЙ collection_id напряму (закешований у storage під час
    опитування), в обхід перевірки назви.

    Повертає сирі лістинги (list[dict]) з полями name, price, attributes
    (model/symbol/backdrop + rarity_per_mille), tg_id тощо.
    """
    _require_lib()
    auth = _require_auth()
    import aportalsmp.gifts as gifts_module

    if sort not in gifts_module.SORTS:
        raise PortalsError(f"Невідомий sort: {sort}. Доступні: {list(gifts_module.SORTS.keys())}")

    url = (
        gifts_module.API_URL
        + "nfts/search?"
        + f"offset={offset}"
        + f"&limit={limit}"
        + gifts_module.SORTS[sort]
        + f"&collection_ids={collection_id}"
        + "&status=listed"
        + "&exclude_bundled=true"
        + "&premarket_status=all"
    )
    headers = {**gifts_module.HEADERS_MAIN, "Authorization": auth}
    response = await gifts_module.fetch(
        method="GET", url=url, headers=headers, impersonate="chrome110"
    )
    ...
    if response.status_code == 429:
        if _retries <= 0:
            raise PortalsError(
                "Portals API повернув статус 429 (забагато запитів) навіть "
                "після повторних спроб."
            )
        await asyncio.sleep(3)
        return await search_listings(collection_id, sort, limit, offset, _retries=_retries - 1)

    if response.status_code == 401:
        raise PortalsAuthError(
            "authData застарів (401 auth sign is invalid). Онови через /setauth "
            "або /refreshauth."
        )
    if response.status_code != 200:
        raise PortalsError(f"Portals API повернув статус {response.status_code}")
